...
let _iaProd = []; // productos detectados
...
window.resetModalIA = function() {
  _iaProd = [];
  $('ia-file').value               = '';
  $('ia-estado').textContent       = '';
  $('ia-estado').style.color       = 'var(--muted)';
  $('ia-zona-upload').style.display    = 'block';
  $('ia-zona-resultado').style.display = 'none';
  $('ia-tbody').innerHTML          = '';
  $('ia-msg').innerHTML            = '';
  $('ia-dropzone').style.opacity   = '1';
  $('ia-dropzone').style.pointerEvents = 'auto';
};

$('ia-file').addEventListener('change', async function(e) {
  const file = e.target.files[0];
  if (!file) return;

  const estado = $('ia-estado');
  estado.textContent = '⏳ Analizando factura...';
  $('ia-dropzone').style.opacity      = '0.4';
  $('ia-dropzone').style.pointerEvents = 'none';

  try {
    const fd = new FormData();
    fd.append('foto', file);

    const res  = await fetch('/api/ia/leer', { method: 'POST', body: fd });
    const data = await res.json();
    if (!data.ok) throw new Error(data.error || 'Error al procesar');

    _iaProd = data.productos;
    renderTablaIA();

    $('ia-zona-upload').style.display    = 'none';
    $('ia-zona-resultado').style.display = 'block';
    $('ia-resumen').textContent =
      `✓ ${_iaProd.length} productos detectados — revisa y edita antes de guardar`;
...
function renderTablaIA() {
  $('ia-tbody').innerHTML = _iaProd.map((p, i) => `
    <tr>
      <td>
        <input class="ia-inp" type="text" value="${p.nombre}"
          onchange="_iaProd[${i}].nombre=this.value"/>
      </td>
      <td>
        <input class="ia-inp" type="text" value="${p.codigo_barras}"
          onchange="_iaProd[${i}].codigo_barras=this.value" style="width:100px"/>
      </td>
      <td>
        <input class="ia-inp" type="number" value="${p.precio_compra}"
          onchange="_iaProd[${i}].precio_compra=+this.value;_iaProd[${i}].precio_venta=+this.value+10;document.getElementById('iapv${i}').textContent=fmtCOP(+this.value+10)"
          style="width:90px"/>
      </td>
      <td>
        <span id="iapv${i}" style="color:var(--teal);font-weight:600">
          ${fmtCOP(p.precio_venta)}
        </span>
      </td>
      <td>
        <input class="ia-inp" type="number" value="${p.stock}"
          onchange="_iaProd[${i}].stock=+this.value;_iaProd[${i}].stock_minimo=Math.floor(+this.value/3);document.getElementById('iasm${i}').textContent=Math.floor(+this.value/3)"
          style="width:70px"/>
      </td>
      <td>
        <span id="iasm${i}" style="color:var(--muted)">${p.stock_minimo}</span>
      </td>
      <td>
        <button onclick="_iaProd.splice(${i},1);renderTablaIA()" style="
...
window.guardarTodosIA = async function() {
  if (!_iaProd.length) return;
  const btn = $('ia-btn-guardar');
  btn.disabled    = true;
  btn.textContent = 'Guardando...';

  let ok = 0, fail = 0;

  for (const p of _iaProd) {
    try {
  ...
  const msg = $('ia-msg');
  if (fail === 0) {
    msg.innerHTML = `<div class="msg ok">✓ ${ok} productos guardados en el inventario.</div>`;
    setTimeout(() => { closeModal('modal-ia'); loadInventario(); }, 1800);
  } else {
    msg.innerHTML = `<div class="msg warn">⚠ ${ok} guardados · ${fail} con error.</div>`;
  }
